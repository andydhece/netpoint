...
const MaintenanceView = () => {
  ...
  const hasRightPanel = isScheduleOpen || !!activeReportMnt;
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        {currentUser.role === 'admin' && (
          <button
            onClick={() => { setActiveReportMnt(null); setIsScheduleOpen(true); }}
      ...
      <div className="flex gap-6 h-full items-start">
        {/* Left Column: Main List Container */}
        <div className={`transition-all duration-300 ${hasRightPanel ? 'w-3/5' : 'w-full'} bg-white dark:bg-[#0c0c0f] border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden shadow-sm p-6`}>
                    ...
                    <div key={mnt.id} className="p-4 border border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-50/30 dark:bg-zinc-900/10 space-y-4">
                      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2 pb-3 border-b border-zinc-150 dark:border-zinc-800/80">
                        <div>
                          ...
                          <h3 className="font-bold text-sm text-zinc-955 dark:text-zinc-50 mt-1">{mnt.title}</h3>
                        </div>
                            ...
                            <div className="text-center">
                              ...
                              <div className="w-20 h-16 rounded border border-zinc-200 dark:border-zinc-800 bg-zinc-100 overflow-hidden flex items-center justify-center">
                                {mnt.photoUrl === 'simulated_battery_photo' ? (
                                  ...
                                  <img src={getImageUrl(mnt.photoUrl)} className="w-full h-full object-cover" alt="Bukti Lapangan" />
                                )}
                              </div>
                            </div>
                          )}
                            ...
                            <div className="text-center">
                              ...
                              <div className="w-20 h-16 rounded border border-zinc-200 dark:border-zinc-800 bg-white overflow-hidden flex items-center justify-center p-1">
                                {mnt.signatureUrl === 'simulated_sig' ? (
                                  ...
                                  <img src={getImageUrl(mnt.signatureUrl)} className="w-full h-full object-contain" alt="Verifikasi" />
                                )}
                              </div>
                            </div>
                          )}
                ...
                <div className="flex items-start justify-between p-5 border-b border-zinc-100 dark:border-zinc-800 sticky top-0 bg-white dark:bg-[#0c0c0f] z-10">
                  <div>
                    <div className="text-[10px] font-bold text-[#059669] uppercase tracking-widest flex items-center gap-1 mb-1">
                      <Plus className="w-3 h-3" /> Agenda Baru
                    </div>
                    <h2 className="text-base font-extrabold text-zinc-900 dark:text-zinc-50">Jadwalkan Pemeliharaan</h2>
                  </div>
                  <button onClick={() => setIsScheduleOpen(false)} className="p-1 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-500 rounded-lg mt-0.5">
                    <X className="w-4 h-4" />
                  </button>
                </div>

                <form onSubmit={handleScheduleSubmit} className="p-5 space-y-4 flex-1">
                  <div>
                    <label className="block text-xs font-medium text-zinc-500 mb-1.5">Judul Kegiatan</label>
                    <input
                      type="text" required value={schedForm.title}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        type="date" required value={schedForm.scheduledDate}
                    ...
                    <div>
                      <label className="block text-xs font-medium text-zinc-500 mb-1.5">Nama PJ</label>
                      <input
                        type="text" required value={schedForm.performedBy}
            ...
            {activeReportMnt && (
              <>
                ...
                <div className="flex items-start justify-between p-5 border-b border-zinc-100 dark:border-zinc-800 sticky top-0 bg-white dark:bg-[#0c0c0f] z-10">
                  <div>
                    <div className="text-[10px] font-bold text-[#059669] uppercase tracking-widest flex items-center gap-1 mb-1">
                      <Plus className="w-3 h-3" /> Laporan Penyelesaian
                    </div>
                    <h2 className="text-base font-extrabold text-zinc-900 dark:text-zinc-50">Laporan Hasil Audit</h2>
                    <p className="text-[11px] text-zinc-400 mt-1">{activeReportMnt.id}: {activeReportMnt.title}</p>
                  </div>
                  <button onClick={() => setActiveReportMnt(null)} className="p-1 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-500 rounded-lg mt-0.5">
                    <X className="w-4 h-4" />
                  </button>
                </div>

                <form onSubmit={handleReportSubmit} className="p-5 space-y-4 flex-1">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-medium text-zinc-500 mb-1.5">Nama Teknisi PJ</label>
                      <input
                        type="text" required value={reportForm.performedBy}
                        onChange={(e) => setReportForm({ ...reportForm, performedBy: e.target.value })}
                        className="w-full bg-zinc-50 dark:bg-[#09090b] border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-2 text-xs shadow-sm focus:outline-none focus:ring-2 focus:ring-[#059669] text-zinc-950 dark:text-zinc-50"
                      />
                    </div>
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-zinc-50 dark:bg-[#09090b] border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-2 text-xs shadow-sm focus:outline-none focus:ring-2 focus:ring-[#059669] text-zinc-850 dark:text-zinc-100"
                      >
                        <option value="Successful">Berhasil (Normal)</option>
                        <option value="Degraded">Terdegradasi (Warning)</option>
                        <option value="Unsuccessful">Gagal (Luring/Kritis)</option>
                      </select>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      required rows="3" value={reportForm.actionsTaken}
                      onChange={(e) => setReportForm({ ...reportForm, actionsTaken: e.target.value })}
                      className="w-full bg-zinc-50 dark:bg-[#09090b] border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-2 text-xs shadow-sm focus:outline-none focus:ring-2 focus:ring-[#059669] text-zinc-955 dark:text-zinc-55"
                  ...
                  <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3 space-y-2 text-xs">
                    <label className="flex items-center gap-2 font-semibold text-zinc-700 dark:text-zinc-300">
                      <input
                        ...
                        className="w-4 h-4 text-[#059669] border-zinc-350 rounded focus:ring-[#059669]"
                      />
                    ...
                    {reportForm.useSparePart && (
                      <div className="grid grid-cols-2 gap-3 pt-2">
                        <div>
                          <label className="block text-[10px] text-zinc-400 mb-1">Pilih Item</label>
                          <select
                            value={reportForm.sparepartId}
                            onChange={(e) => setReportForm({ ...reportForm, sparepartId: e.target.value })}
                            className="w-full bg-white dark:bg-[#09090b] border border-zinc-200 dark:border-zinc-800 rounded-lg px-2.5 py-1 text-xs text-zinc-850 dark:text-zinc-100"
                          >
                            {spareparts.map(sp => (
                        ...
                        <div>
                          <label className="block text-[10px] text-zinc-400 mb-1">Jumlah (Qty)</label>
                          <input
                            type="number"
                            min="1"
                            ...
                            className="w-full bg-white dark:bg-[#09090b] border border-zinc-200 dark:border-zinc-800 rounded-lg px-2.5 py-1 text-xs text-zinc-950 dark:text-zinc-50 font-mono"
                          />
                        </div>
                      </div>
                    )}
                  ...
                  <div>
                    <label className="block text-xs font-medium text-zinc-500 mb-1">
                      <Camera className="w-3.5 h-3.5 inline mr-1 text-zinc-400" /> Unggah Foto Bukti Kerja
                    </label>
                    <div className="flex items-center gap-3">
                      <button
                        type="button"
                        onClick={() => fileInputRef.current.click()}
                        className="bg-[#ffffff] dark:bg-[#262626] hover:bg-zinc-50 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-200 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-1.5 text-xs font-semibold shadow-sm transition-colors flex items-center gap-1.5"
                      >
                        Pilih Foto
                      </button>
                      <input
                      ...
                      />
                      {photoPreview ? (
                        <div className="w-16 h-12 border border-zinc-200 dark:border-zinc-850 rounded overflow-hidden shadow-inner bg-zinc-50">
                          <img src={photoPreview} className="w-full h-full object-cover" alt="Preview Bukti" />
                        </div>
                      ) : (
                        <span className="text-[10px] text-zinc-450 italic">Belum ada foto.</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-zinc-500 mb-1 flex items-center justify-between">
                      <span><Signature className="w-3.5 h-3.5 inline mr-1 text-zinc-450" /> Tanda Tangan Verifikasi</span>
                      <div className="flex gap-2">
                        <button
                          type="button"
                          onClick={clearCanvas}
                  ...
                  <div className="flex gap-2 pt-2 border-t border-zinc-100 dark:border-zinc-800">
                    <button
                      ...
                      className="flex-1 bg-white dark:bg-[#262626] hover:bg-zinc-50 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-200 border border-zinc-200 dark:border-zinc-700 rounded-lg py-2 text-xs font-semibold"
                    >
                      Batal
                    </button>
                    <button
                      ...
                      className={`flex-1 rounded-lg py-2 text-xs font-semibold shadow-sm transition-colors ${
                        signatureSaved 
